...
@File: generate_api_2.py
@Author: QI
@Date: 2021/6/4 11:40
@Description: Transformer + A_res(In Encoder) + A_tgt(In Decoder before multi_head_attn)
"""
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, A_tgt, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        output = tgt
        for i in range(self.num_layers):
            output = self.layers[i](output, memory, A_tgt, tgt_mask=tgt_mask, memory_mask=memory_mask, tgt_key_padding_mask=tgt_key_padding_mask,
...
class MyTransformer(nn.Module):
    ...
    def forward(self, src, sents_length, tgt, response_length, A_res, A_tgt, tgt_mask=None, src_key_padding_mask=None, tgt_key_padding_mask=None):
        ...
        # 嵌入A_res
        A_res = A_res.unsqueeze(1).repeat(1, args.sents_max_len, 1).permute(1, 0, 2)  # [sents_max_len,batch,user_dim]
        embedded_src = embedded_src.add(A_res)
        ...
        generate_output = self.transformer_decoder(embedded_tgt, memory, A_tgt, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask)  # [tgt_max_len,batch_size,d_model]
    ...
    def greedy_decoder(self, src, sents_length, A_res, A_tgt, start_symbol=Vocab.SOS):
        ...
        for i in range(0, args.target_max_len - 1):
            ...
            dec_output = self.transformer_decoder(cur_dec_inputs, memory, A_tgt)  # [batch_size, tgt_len, d_model]
